...
本项目持续优化中，后续计划参考 EID (External Item Descriptions) 模组的成熟实现，进一步完善数据采集功能。

### 近期计划 (Phase 6)

| 任务 | 优先级 | 说明 |
|-----|--------|------|
| **实体搜索优化** | 🔴 高 | 使用 `FindInRadius` 替代全量遍历，减少 CPU 开销 50%+ |
| **玩家物品通道** | 🔴 高 | 新增 `PLAYER_ITEMS_DETAILED` 通道，采集主动/被动道具、卡牌、药丸等 |
| **事件驱动采集** | 🟡 中 | 使用游戏回调替代轮询，减少无效数据传输 |
| **药丸效果识别** | 🟡 中 | 获取药丸效果 ID 和识别状态 |
| **变身进度追踪** | 🟢 低 | 新增 `PLAYER_TRANSFORMATIONS` 通道 |

### 中期计划 (Phase 7)

| 任务 | 说明 |
|-----|------|
| **条件数据过滤器** | 根据游戏状态动态过滤/转换数据 |
| **数据修改器链** | 参考 EID 的 DescModifiers 实现 |
| **智能缓存机制** | 减少重复计算，优化性能 |
...
| 任务 | 说明 |
|-----|------|
| **RNG 预测系统** | 参考 EID 的 Xorshift 实现，预测随机道具效果 |
| **Bag of Crafting 集成** | 合成袋配方计算 (Tainted Cain) |
| **REPENTOGON 支持** | 利用扩展 API 获取更多数据 |

### 兼容性承诺

- ✅ **向后兼容**：新功能使用新通道名，不修改现有 API
- ✅ **字段只增不删**：现有数据结构保持不变
- ✅ **分阶段发布**：v2.1.x → v2.2.0 → v2.3.0 → v3.0.0

> 📚 详细计划请参考 [REFACTORING_PLAN.md](REFACTORING_PLAN.md)
