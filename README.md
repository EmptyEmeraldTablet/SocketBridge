# SocketBridge 项目说明文档

## 📋 项目概述

**SocketBridge** 是一个为《以撒的结合：重生》（The Binding of Isaac: Repentance）游戏开发的模组，实现了游戏与 Python 程序之间的实时数据桥接。该项目旨在为游戏 AI 开发、数据分析和自动化测试提供强大的基础设施。

### 核心特性

- ✅ **实时数据采集** - 高效采集游戏内各类数据（玩家、敌人、投射物、房间等）
- ✅ **多频率采集** - 支持高频/中频/低频/变化时四种采集模式
- ✅ **双向通信** - 游戏数据实时传输到 Python，Python 可发送控制指令回游戏
- ✅ **事件系统** - 完整的游戏事件监听与回调机制
- ✅ **AI 控制支持** - 内置 AI 控制框架，支持手动/AI 模式切换（F3 键）
- ✅ **数据记录** - 内置数据记录器，支持游戏会话录制与回放
- ✅ **模块化设计** - 可扩展的收集器注册系统，易于添加新的数据通道

---

## 🏗️ 项目架构

### 整体架构图

```
┌─────────────────────────────────────────────────────────────┐
│                    The Binding of Isaac                      │
│                      (游戏进程)                               │
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              main.lua (游戏模组)                       │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌────────────┐ │   │
│  │  │ Collector    │  │   Network    │  │   Event    │ │   │
│  │  │  Registry    │  │   Layer      │  │   System   │ │   │
│  │  └──────────────┘  └──────────────┘  └────────────┘ │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌────────────┐ │   │
│  │  │   Protocol    │  │   Input      │  │   Command  │ │   │
│  │  │    Layer      │  │  Executor    │  │  Handler   │ │   │
│  │  └──────────────┘  └──────────────┘  └────────────┘ │   │
│  └──────────────────────────────────────────────────────┘   │
│                          │                                   │
│                          │ TCP Socket (127.0.0.1:9527)      │
│                          ▼                                   │
└─────────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────────┐
│                    Python 进程                               │
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              isaac_bridge.py (核心桥接)               │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌────────────┐ │   │
│  │  │   Network    │  │   GameState  │  │   Event    │ │   │
│  │  │   Server     │  │   Manager    │  │   System   │ │   │
│  │  └──────────────┘  └──────────────┘  └────────────┘ │   │
│  └──────────────────────────────────────────────────────┘   │
│                          │                                   │
│          ┌───────────────┼───────────────┐                  │
│          ▼               ▼               ▼                  │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐        │
│  │ example_ai   │ │data_recorder │ │  自定义模块   │        │
│  │   (AI控制)   │ │  (数据记录)  │ │              │        │
│  └──────────────┘ └──────────────┘ └──────────────┘        │
└─────────────────────────────────────────────────────────────┘
```

### 文件结构

```
SocketBridge/
├── main.lua                 # 游戏模组主文件（Lua）
├── metadata.xml             # 模组元数据
└── python/                  # Python 端代码
    ├── isaac_bridge.py      # 核心桥接库
    ├── data_recorder.py     # 数据记录器
    ├── example_ai.py        # AI 控制示例
    ├── logs/                # 日志目录
    └── recordings/          # 录制数据目录
```

---

## 🔧 核心模块详解

### 1. Lua 端 (main.lua)

#### 1.1 配置系统 (Config)

```lua
Config = {
    HOST = "127.0.0.1",      -- 服务器地址
    PORT = 9527,             -- 服务器端口
    
    -- 采集频率配置（帧数间隔）
    CollectIntervals = {
        HIGH = 1,            -- 每帧采集
        MEDIUM = 5,          -- 5帧一次
        LOW = 30,            -- 30帧一次
        ON_CHANGE = -1       -- 仅在变化时采集
    },
}
```

#### 1.2 收集器注册系统 (CollectorRegistry)

这是整个数据采集的核心，采用注册表模式管理所有数据收集器。

**主要功能：**
- 动态注册数据收集器
- 支持启用/禁用单个收集器
- 支持动态调整采集频率
- 自动缓存和变化检测

**已注册的数据通道：**

| 通道名称 | 采集频率 | 优先级 | 说明 |
|---------|---------|--------|------|
| PLAYER_POSITION | HIGH | 10 | 玩家位置、速度、朝向 |
| PLAYER_STATS | LOW | 5 | 玩家属性（伤害、速度、射程等） |
| PLAYER_HEALTH | ON_CHANGE | 8 | 玩家生命值 |
| PLAYER_INVENTORY | ON_CHANGE | 3 | 玩家物品栏、道具、消耗品 |
| ENEMIES | MEDIUM | 7 | 敌人信息（位置、血量、状态） |
| PROJECTILES | HIGH | 9 | 投射物（敌人投射物、玩家眼泪、激光） |
| ROOM_INFO | MEDIUM | 4 | 房间信息（类型、清除状态、敌人数量） |
| ROOM_LAYOUT | ON_CHANGE | 2 | 房间布局（障碍物、门） |

#### 1.3 网络层 (Network)

负责与 Python 端的 TCP 通信。

**主要方法：**
- `connect()` - 连接到 Python 服务器
- `disconnect()` - 断开连接
- `send(data)` - 发送 JSON 数据
- `receive()` - 接收 JSON 数据

#### 1.4 协议层 (Protocol)

定义消息格式和类型。

**消息类型：**
- `DATA` - 增量数据更新
- `FULL_STATE` - 完整状态快照
- `EVENT` - 游戏事件
- `CMD` - 命令响应

**消息格式示例：**
```json
{
  "version": "2.0",
  "type": "DATA",
  "timestamp": 1234567890,
  "frame": 1234,
  "room_index": 42,
  "payload": {
    "PLAYER_POSITION": {...},
    "ENEMIES": [...]
  },
  "channels": ["PLAYER_POSITION", "ENEMIES"]
}
```

#### 1.5 输入执行器 (InputExecutor)

接收来自 Python 的控制指令并执行游戏输入。

**支持的输入类型：**
- 移动方向（上下左右）
- 射击方向（上下左右）
- 使用道具
- 放置炸弹
- 使用卡牌/药丸
- 丢弃物品

#### 1.6 事件系统 (EventSystem)

监听游戏内事件并转发到 Python。

**支持的事件：**
- `GAME_START` - 游戏开始
- `GAME_END` - 游戏结束
- `ROOM_CLEAR` - 房间清除
- `PLAYER_DAMAGE` - 玩家受伤
- `NPC_DEATH` - NPC 死亡
- `PLAYER_DEATH` - 玩家死亡
- `ITEM_COLLECTED` - 获得道具

#### 1.7 手动/AI 模式切换

- 按 **F3** 键切换手动/AI 控制模式
- AI 模式下，当房间内有敌人时自动启用 AI 控制
- 手动模式下，完全由玩家控制

---

### 2. Python 端 (isaac_bridge.py)

#### 2.1 IsaacBridge 类

核心桥接类，负责与游戏通信。

**主要功能：**
- TCP 服务器（监听 127.0.0.1:9527）
- 维护实时游戏状态
- 事件回调系统
- 发送控制指令

**核心方法：**

```python
# 启动/停止服务器
bridge.start()
bridge.stop()

# 注册事件回调
@bridge.on("event:PLAYER_DAMAGE")
def on_player_damage(data):
    print(f"Player took {data['amount']} damage")

# 发送控制指令
bridge.send_input(move_dir=(1, 0), shoot_dir=(0, -1))
bridge.use_item()
bridge.use_bomb()
```

#### 2.2 GameState 类

游戏状态容器，支持增量更新。

```python
@dataclass
class GameState:
    data: Dict[str, Any]           # 各通道数据
    last_update: Dict[str, int]    # 最后更新帧
    frame: int                     # 当前帧
    room_index: int                # 当前房间索引
```

#### 2.3 GameDataAccessor 类

提供便捷的游戏数据访问接口。

```python
data = GameDataAccessor(bridge)

# 获取玩家位置
player_pos = data.get_player_position()

# 获取敌人列表
enemies = data.get_enemies()

# 获取投射物
projectiles = data.get_enemy_projectiles()

# 获取房间信息
room_info = data.get_room_info()
```

#### 2.4 事件系统

支持多种事件订阅方式：

```python
# 订阅特定事件
@bridge.on("event:GAME_START")
def on_game_start(data):
    print("Game started!")

# 订阅数据通道更新
@bridge.on("data:PLAYER_HEALTH")
def on_health_update(health):
    print(f"Health: {health}")

# 订阅所有数据更新
@bridge.on("data")
def on_data_update(data):
    print(f"Received data: {data}")
```

---

### 3. 数据记录器 (data_recorder.py)

用于记录游戏数据以供后续分析或训练使用。

**主要功能：**
- 自动记录游戏会话
- 帧级数据记录
- 事件记录
- 自动保存（支持 gzip 压缩）

**使用示例：**

```python
from isaac_bridge import IsaacBridge
from data_recorder import GameDataRecorder

bridge = IsaacBridge()
recorder = GameDataRecorder(bridge, output_dir="./recordings")

bridge.start()
# 游戏数据会自动记录
```

**记录的数据格式：**

```json
{
  "session_id": "session_20240106_123456",
  "start_time": 1704537296.0,
  "continued": false,
  "metadata": {},
  "frames": [
    {
      "frame": 1234,
      "timestamp": 1704537296.123,
      "data": {
        "PLAYER_POSITION": {...},
        "ENEMIES": [...]
      }
    }
  ],
  "events": [
    {
      "type": "PLAYER_DAMAGE",
      "frame": 1250,
      "timestamp": 1704537296.456,
      "data": {...}
    }
  ]
}
```

---

### 4. AI 控制示例 (example_ai.py)

演示如何使用 IsaacBridge 实现简单的 AI 控制逻辑。

**SimpleAI 类：**

```python
class SimpleAI:
    def __init__(self, bridge: IsaacBridge):
        self.bridge = bridge
        self.data = GameDataAccessor(bridge)
        self.action_interval = 3  # 每3帧决策一次
    
    def update(self):
        """每帧更新，返回 (移动方向, 射击方向)"""
        # 获取玩家位置
        player_pos = self.data.get_player_position()
        
        # 获取敌人和投射物
        enemies = self.data.get_enemies()
        projectiles = self.data.get_enemy_projectiles()
        
        # 决策
        move_dir = self._decide_movement(...)
        shoot_dir = self._decide_shooting(...)
        
        return move_dir, shoot_dir
```

**AI 策略：**
- **移动决策** - 远离最近的危险（投射物和敌人）
- **射击决策** - 向最近的敌人射击

---

## 🚀 快速开始

### 环境要求

- 《以撒的结合：重生》游戏
- Python 3.8+
- Python 依赖：无（仅使用标准库）

### 安装步骤

1. **安装游戏模组**
   - 将 `SocketBridge` 文件夹复制到游戏 mods 目录
   - 在游戏中启用 SocketBridge 模组

2. **启动 Python 端**
   ```bash
   cd python
   python isaac_bridge.py
   ```

3. **启动游戏**
   - 启动《以撒的结合：重生》
   - 模组会自动连接到 Python 服务器

### 基本使用

#### 1. 接收游戏数据

```python
from isaac_bridge import IsaacBridge

bridge = IsaacBridge()

@bridge.on("data")
def on_data(data):
    print(f"Frame {bridge.state.frame}: {data}")

bridge.start()
```

#### 2. 控制 AI

```python
from isaac_bridge import IsaacBridge, GameDataAccessor
import time

bridge = IsaacBridge()
data = GameDataAccessor(bridge)

@bridge.on("data")
def on_update(data):
    # 获取玩家位置
    player = data.get_player_position()
    
    # 获取最近的敌人
    enemies = data.get_enemies()
    if enemies:
        nearest = min(enemies, key=lambda e: e['distance'])
        
        # 向敌人移动并射击
        dx = nearest['pos']['x'] - player['pos']['x']
        dy = nearest['pos']['y'] - player['pos']['y']
        
        move_dir = (1 if dx > 0 else -1 if dx < 0 else 0,
                    1 if dy > 0 else -1 if dy < 0 else 0)
        shoot_dir = move_dir
        
        bridge.send_input(move_dir=move_dir, shoot_dir=shoot_dir)

bridge.start()
```

#### 3. 记录游戏数据

```python
from isaac_bridge import IsaacBridge
from data_recorder import GameDataRecorder

bridge = IsaacBridge()
recorder = GameDataRecorder(bridge)

bridge.start()
# 游戏数据会自动记录到 recordings/ 目录
```

---

## 📊 数据通道详细说明

### PLAYER_POSITION

**采集频率：** HIGH（每帧）

**数据结构：**
```json
{
  "0": {
    "pos": {"x": 100.0, "y": 200.0},
    "vel": {"x": 1.5, "y": 0.0},
    "move_dir": 1,
    "fire_dir": 2,
    "head_dir": 1,
    "aim_dir": {"x": 0.8, "y": 0.6}
  }
}
```

### PLAYER_STATS

**采集频率：** LOW（30帧）

**数据结构：**
```json
{
  "0": {
    "player_type": 0,
    "damage": 3.5,
    "speed": 1.0,
    "tears": 15,
    "range": 220.0,
    "tear_range": 5.5,
    "shot_speed": 1.0,
    "luck": 1.0,
    "tear_height": -6.0,
    "tear_falling_speed": 1.5,
    "can_fly": false,
    "size": 10.0,
    "sprite_scale": 1.0
  }
}
```

### PLAYER_HEALTH

**采集频率：** LOW（30帧）

**数据结构：**
```json
{
  "0": {
    "red_hearts": 6,
    "max_hearts": 6,
    "soul_hearts": 2,
    "black_hearts": 0,
    "bone_hearts": 0,
    "golden_hearts": 0,
    "eternal_hearts": 0,
    "rotten_hearts": 0,
    "broken_hearts": 0,
    "extra_lives": 0
  }
}
```

### ENEMIES

**采集频率：** HIGH（每帧）

**数据结构：**
```json
[
  {
    "id": 12345,
    "type": 10,
    "variant": 0,
    "subtype": 0,
    "pos": {"x": 300.0, "y": 200.0},
    "vel": {"x": 0.5, "y": 0.0},
    "hp": 20.0,
    "max_hp": 20.0,
    "is_boss": false,
    "is_champion": false,
    "state": 3,
    "state_frame": 10,
    "projectile_cooldown": 30,
    "projectile_delay": 60,
    "collision_radius": 15.0,
    "distance": 250.0,
    "target_pos": {"x": 100.0, "y": 200.0},
    "v1": {"x": 0.0, "y": 0.0},
    "v2": {"x": 0.0, "y": 0.0}
  }
]
```

### PROJECTILES

**采集频率：** HIGH（每帧）

**数据结构：**
```json
{
  "enemy_projectiles": [
    {
      "id": 67890,
      "pos": {"x": 250.0, "y": 180.0},
      "vel": {"x": -2.0, "y": 0.0},
      "variant": 0,
      "collision_radius": 8.0,
      "height": 0.0,
      "falling_speed": 0.0,
      "falling_accel": 0.0
    }
  ],
  "player_tears": [
    {
      "id": 11111,
      "pos": {"x": 120.0, "y": 200.0},
      "vel": {"x": 3.0, "y": 0.0},
      "variant": 0,
      "collision_radius": 6.0,
      "height": 0.0,
      "scale": 1.0
    }
  ],
  "lasers": [
    {
      "id": 22222,
      "pos": {"x": 300.0, "y": 200.0},
      "angle": 0.0,
      "max_distance": 400.0,
      "is_enemy": true
    }
  ]
}
```

### ROOM_INFO

**采集频率：** LOW

**数据结构：**
```json
{
  "room_type": 1,
  "room_shape": 1,
  "room_index": 42,
  "stage": 1,
  "stage_type": 0,
  "difficulty": 0,
  "is_clear": false,
  "is_first_visit": true,
  "grid_width": 13,
  "grid_height": 7,
  "top_left": {"x": 280.0, "y": 140.0},
  "bottom_right": {"x": 560.0, "y": 420.0},
  "has_boss": false,
  "enemy_count": 5,
  "room_variant": 0
}
```

### ROOM_LAYOUT

**采集频率：** LOW

**数据结构：**
```json
{
  "grid": {
    "42": {
      "type": 2,
      "variant": 0,
      "state": 0,
      "collision": 1,
      "x": 280.0,
      "y": 140.0
    }
  },
  "doors": {
    "0": {
      "target_room": 41,
      "target_door_slot": 2,
      "state": 0,
      "variant": 0
    }
  }
}
```

---

## 🎮 控制指令

### 发送输入指令

```python
# 移动和射击
bridge.send_input(
    move_dir=(x, y),    # x, y ∈ {-1, 0, 1}
    shoot_dir=(x, y)     # x, y ∈ {-1, 0, 1}
)

# 使用主动道具
bridge.use_item()

# 放置炸弹
bridge.use_bomb()

# 使用卡牌/药丸
bridge.use_card()
bridge.use_pill()

# 丢弃物品
bridge.drop()
```

### 控制指令格式

```json
{
  "version": "2.0",
  "type": "CMD",
  "timestamp": 1234567890,
  "input": {
    "move_dir": {"x": 1, "y": 0},
    "shoot_dir": {"x": 0, "y": -1},
    "use_item": false,
    "use_bomb": false,
    "use_card": false,
    "use_pill": false,
    "drop": false
  }
}
```

---

## 🔌 事件系统

### 游戏事件

| 事件名称 | 触发时机 | 数据结构 |
|---------|---------|---------|
| GAME_START | 游戏开始 | `{"continued": bool}` |
| GAME_END | 游戏结束 | `{"reason": str}` |
| ROOM_CLEAR | 房间清除 | `{"room_index": int}` |
| PLAYER_DAMAGE | 玩家受伤 | `{"amount": float, "flags": int, "source_type": int, "hp_after": int}` |
| NPC_DEATH | NPC 死亡 | `{"type": int, "variant": int, "subtype": int, "pos": dict, "is_boss": bool}` |
| PLAYER_DEATH | 玩家死亡 | `{"player_idx": int}` |
| ITEM_COLLECTED | 获得道具 | `{"item_id": int, "first_time": bool, "slot": int, "player_idx": int}` |

### 事件订阅示例

```python
@bridge.on("event:PLAYER_DAMAGE")
def on_damage(data):
    print(f"Player took {data['amount']} damage!")

@bridge.on("event:NPC_DEATH")
def on_npc_death(data):
    if data['is_boss']:
        print("Boss defeated!")
```

---

## 🛠️ 高级功能

### 1. 动态配置数据通道

```python
# 禁用某个数据通道
bridge.set_channel_enabled("PLAYER_STATS", False)

# 调整采集频率
bridge.set_channel_interval("ENEMIES", "HIGH")

# 获取当前配置
config = bridge.get_channel_config()
```

### 2. 自定义数据收集器（Lua 端）

```lua
-- 在 main.lua 中添加自定义收集器
CollectorRegistry:register("CUSTOM_DATA", {
    interval = "MEDIUM",
    priority = 6,
    collect = function()
        -- 自定义采集逻辑
        return {
            custom_field = Game():GetFrameCount()
        }
    end
})
```

### 3. 批量数据查询

```python
# 获取完整状态
full_state = bridge.state.get_full_state()

# 获取多个通道数据
data = {
    "player": bridge.state.get("PLAYER_POSITION"),
    "enemies": bridge.state.get("ENEMIES"),
    "room": bridge.state.get("ROOM_INFO")
}
```

### 4. 性能优化

```python
# 只订阅需要的数据通道
@bridge.on("data:PLAYER_POSITION")
def on_position(pos):
    # 只处理玩家位置更新
    pass

# 使用事件队列异步处理
while True:
    event = bridge.event_queue.get()
    if event.type == "PLAYER_DAMAGE":
        # 处理伤害事件
        pass
```

---

## 📈 性能指标

### 数据采集性能

| 数据通道 | 采集频率 | 数据量 | CPU 占用 |
|---------|---------|--------|---------|
| PLAYER_POSITION | 60 fps | ~200 B | < 1% |
| ENEMIES | 12 fps | ~2 KB | < 2% |
| PROJECTILES | 60 fps | ~1 KB | < 1% |
| ROOM_INFO | 12 fps | ~300 B | < 1% |

### 网络性能

- **协议：** TCP
- **端口：** 9527
- **延迟：** < 5ms（本地）
- **吞吐量：** ~500 KB/s（全通道开启）

---

## 🐛 故障排除

### 常见问题

1. **连接失败**
   - 确认 Python 服务器已启动
   - 检查端口 9527 是否被占用
   - 查看游戏控制台日志

2. **数据不更新**
   - 检查数据通道是否启用
   - 确认采集频率设置
   - 查看事件回调是否正确注册

3. **AI 控制不生效**
   - 确认 AI 模式已启用（按 F3 切换）
   - 检查控制指令是否正确发送
   - 确认房间内有敌人（AI 仅在战斗时激活）

### 调试技巧

```python
# 启用详细日志
import logging
logging.basicConfig(level=logging.DEBUG)

# 查看统计信息
print(bridge.stats)

# 查看当前状态
print(bridge.state.get_full_state())
```

---

## 📝 开发路线图

### 已完成 ✅

- ✅ 基础数据采集框架
- ✅ 网络通信层
- ✅ 事件系统
- [x] AI 控制支持
- [x] 数据记录器
- ✅ 手动/AI 模式切换

### 计划中 🚧

- [ ] 更多数据通道（道具效果、诅咒等）
- [ ] 可视化调试工具
- [ ] 强化学习集成
- [ ] 多玩家支持
- [ ] 性能优化

---

## 📄 许可证

本项目仅供学习和研究使用。

---

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

---

## 📧 联系方式

如有问题或建议，请通过 GitHub Issues 联系。

---

**最后更新：** 2026年1月6日
**版本：** 2.0
