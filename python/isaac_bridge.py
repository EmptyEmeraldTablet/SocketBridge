...
class IsaacBridge:
        ...
        while self.running and self.connected and self.client:
            try:
                ...
                if time.time() - last_data_time > heartbeat_interval:
                    logger.warning("No data received for 5 seconds, game may have exited")
                    break
                continue
    ...
    def send_input(self, move: Tuple[int, int] = None, shoot: Tuple[int, int] = None,
                   use_item: bool = None, use_bomb: bool = None,
                   use_card: bool = None, use_pill: bool = None,
                   drop: bool = None):
        """
        发送输入指令
    ...
    def set_interval(self, channel: str, interval: CollectInterval):
        """设置数据通道采集频率"""
        return self.send_command("SET_INTERVAL", {"channel": channel, "interval": interval.value})
        ...
        while True:
            ...
            if bridge.is_connected():
                ...
                if frame_count % 5 == 0:
                    ...
                    pos_str = f"({pos['pos']['x']:.0f}, {pos['pos']['y']:.0f})" if pos else "N/A"
                    logger.info(
