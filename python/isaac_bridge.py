...
class IsaacBridge:
    ...
    def _receive_loop(self):
        ...
        heartbeat_interval = 10.0  # 10秒没有数据视为断开（更长的超时）
        receive_timeout = 1.0  # 每次 recv 的超时时间

        while self.running and self.connected and self.client:
            # 每次循环开始时检查 socket 是否仍然有效
            try:
                if self.client.fileno() < 0:
                    logger.warning(
                        "Socket file descriptor is invalid, stopping receive loop"
                    )
                    break
            except (OSError, Exception):
                logger.warning("Socket check failed, stopping receive loop")
                break

            try:
                self.client.settimeout(receive_timeout)
                data = self.client.recv(65536)

                if not data or len(data) == 0:
                    # 对方关闭连接（游戏正常退出或空数据包）
                    logger.info("Game closed connection (received empty data)")
                    break
                ...
                if time.time() - last_data_time > heartbeat_interval:
                    logger.warning(
                        f"No data received for {heartbeat_interval} seconds, connection may be dead"
                    )
                    break
                continue
                ...
                if e.errno == 10054:  # WSAECONNRESET
                    ...
                    logger.info("Socket was closed unexpectedly")
                else:
                    logger.error(f"Network error (errno={e.errno}): {e}")
                break
            except Exception as e:
    ...
    def is_connected(self) -> bool:
        ...
        # 3. 检查 socket 是否仍然有效（通过文件描述符）
        try:
            # fileno() 返回 -1 表示 socket 已关闭
            if self.client.fileno() < 0:
                return False
        except (OSError, Exception):
            return False

        # 4. 额外检查：尝试获取 socket 错误状态
        try:
            # 使用 getsockopt 检查 socket 错误
            error = self.client.getsockopt(socket.SOL_SOCKET, socket.SO_ERROR)
            if error != 0:
                return False
        except (OSError, Exception):
            pass
