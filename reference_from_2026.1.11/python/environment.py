...
根据 reference.md 和 DATA_PROTOCOL.md 中的环境数据格式设计。
"""
...
class DangerZone:
    ...
    intensity: float = 1.0  # 危险强度 0-1
    estimated_frames: int = 60  # 预计持续帧数
...
class GameMap:
    ...
    def __init__(self, grid_size: float = 91.0, width: int = 13, height: int = 7):
        """
        初始化地图
        ...
        # 门位置
        self.doors: Dict[int, Tuple[Vector2D, Vector2D]] = {}  # slot -> (min, max)
    ...
    def update_from_layout(self, layout: RoomLayout):
        """从房间布局数据更新地图"""
        if layout is None:
            return
        ...
        self.pixel_width = layout.width * layout.grid_size
        self.pixel_height = layout.height * layout.grid_size
        ...
        # 更新网格
        for idx, tile in layout.tiles.items():
            gx = idx % self.width
            gy = idx // self.width
        ...
        # 更新门
        for slot, door in layout.doors.items():
            self._update_door_position(slot, door)

    def _update_door_position(self, slot: int, door: DoorData):
        """更新门位置"""
        # 门是开口，不是障碍物
        # 门位置实际上是可通行的
        pass
...
class EnvironmentModel:
    ...
    def update_room(
        ...
        # 更新静态地图
        if room_layout:
            self.game_map.update_from_layout(room_layout)
