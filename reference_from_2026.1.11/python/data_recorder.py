...
class GameDataRecorder:
    ...
    def _record_frame(self, data: Dict):
        """记录一帧数据"""
        frame_data = {
            "frame": self.data.frame,
            "room_index": self.data.room_index,
            "timestamp": time.time(),
            "data": data,
        }
...
class DataInspector:
    ...
    def __init__(self, bridge: IsaacBridge, interval: float = 10.0, log_file: Optional[str] = None):
        """
        Args:
    ...
    def _extract_key_info(self, channel: str, item: Dict) -> str:
        ...
        if channel in ["PLAYER_POSITION"]:
            ...
            return f"coins={coins}, bombs={bombs}, keys={keys}, {items_str}, total={count}"
...
class DataReplayer:
    ...
    def load_session(self, session_id: str) -> Dict:
        ...
        # 加载帧数据
        frames = []
        for chunk_file in sorted(self.recordings_dir.glob(f"{session_id}_chunk_*.json.gz")):
...
def main():
    ...
    parser.add_argument("--mode", choices=["record", "inspect", "both"], default="both",
    ...
    if args.mode in ["inspect", "both"]:
        log_file = args.log_file or f"./logs/data_inspect_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    if inspector:
        logger.info(f"Data inspection interval: {args.interval}s, log file: {inspector.log_file}")
